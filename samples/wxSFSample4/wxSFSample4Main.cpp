...
void wxSFSample4Frame::OnRightClickCanvas(wxMouseEvent& event)
{
    ...
    if(pShape)
    {
        ...
        // is resized in accordance to dimensions of it parent control shape or vice versa.
        pShape->SetControl(CreateGUIControl(), sfFIT_SHAPE_TO_CONTROL);

        // You can spedify whether events generated by managed control are processes the shape canvas
        // (in this case use FALSE value) or the widget itself as well (use TRUE value)
        pShape->SetEventProcessing(false);
    }
...
{
    m_nControlType = ( m_nControlType + 1 ) % 4;

    switch( m_nControlType )
    {
        case 0:
            return new wxButton(m_pCanvas, wxID_ANY, wxT("Hello World!"));

        case 1:
            return new wxTextCtrl(m_pCanvas, wxID_ANY, wxT("Hello World!"), wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE);

        case 2:
            return new wxSlider(m_pCanvas, wxID_ANY, 50, 0, 100);

        case 3:
        {
            wxGauge *pCtrl = new wxGauge(m_pCanvas, wxID_ANY, 50);
            pCtrl->SetValue(25);
            return pCtrl;
        }
